...
def create_container_from_package(package, observables, base_connector):
    ...
    if (artifacts):
        ...
        container['sensitivity'] = get_container_sensitivity(package['input_data'], base_connector)
...
def get_container_sensitivity(input_data, base_connector):
    ...
    try:
        ...
        resp_json = json.loads(json.loads(input_data))

        # Strip unnecessary values
        resp_json = resp_json['stix_header']['handling']

        # Handles unknown list sizes
        for row in resp_json:

            if type(row) is dict and row['marking_structures']:

                # Handles unknown list sizes
                for entry in row['marking_structures']:

                    if type(entry) is dict and entry['color']:

                        return str(entry['color']).lower()
    except:
...
if __name__ == '__main__':
    ...
    with open(sys.argv[1]) as f:
        # Load the stix document
        package = parse_stix(f, None)
        ...
        if (package):
            ...
            with open('./stix-parsed.json', 'w') as f:
